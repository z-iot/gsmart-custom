...
#include "sbus_light.h"
#include "esphome/core/helpers.h"

namespace esphome {
namespace sbus {

static const char *const TAG = "sbus.light";

void SbusLight::setup() {
  if (this->color_temperature_id_.has_value()) {
    this->parent_->register_listener(*this->color_temperature_id_, [this](const SbusDatapoint &datapoint) {
  ...
  if (this->dimmer_id_.has_value()) {
    this->parent_->register_listener(*this->dimmer_id_, [this](const SbusDatapoint &datapoint) {
      auto call = this->state_->make_call();
  ...
  if (rgb_id_.has_value()) {
    this->parent_->register_listener(*this->rgb_id_, [this](const SbusDatapoint &datapoint) {
    ...
    this->parent_->register_listener(*this->hsv_id_, [this](const SbusDatapoint &datapoint) {
...
void SbusLight::dump_config() {
  ESP_LOGCONFIG(TAG, "Sbus Dimmer:");
  if (this->dimmer_id_.has_value())
...
light::LightTraits SbusLight::get_traits() {
...
void SbusLight::setup_state(light::LightState *state) { state_ = state; }

void SbusLight::write_state(light::LightState *state) {
