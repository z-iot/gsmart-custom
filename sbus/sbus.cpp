  ...
  if (len >= 2) {
    ...
    switch (datapoint.type) {
      case SbusDatapointType::RAW:
        ...
        ESP_LOGV(TAG, "Datapoint %u update to %s", datapoint.id, format_hex_pretty(datapoint.value_raw).c_str());
        break;
      case SbusDatapointType::BOOLEAN:
        ...
        ESP_LOGV(TAG, "Datapoint %u update to %s", datapoint.id, ONOFF(datapoint.value_bool));
        break;
      case SbusDatapointType::INTEGER:
        ...
        ESP_LOGV(TAG, "Datapoint %u update to %d", datapoint.id, datapoint.value_int);
        break;
      case SbusDatapointType::STRING:
        datapoint.value_string = std::string(reinterpret_cast<const char *>(data), data_len);
        ESP_LOGV(TAG, "Datapoint %u update to %s", datapoint.id, datapoint.value_string.c_str());
        break;
      default:
