  ...
  if (this->has_sta()) {
    ...
    switch (this->state_) {
      case WIFI_COMPONENT_STATE_COOLDOWN: {
        ...
        if (millis() - this->action_started_ > 30 * 1000) {
...
#ifdef USE_WIFI_AP
//zakaze AP po 2 minutach
    if (now - this->ap_last_start_time_ > 2 * 60000) 
      this->disableAp();
...
void WiFiComponent::add_sta(const WiFiAP &ap) { this->default_sta_.push_back(ap); }
void WiFiComponent::set_sta(const WiFiAP &ap) {
  this->clear_sta();
if (!ap.get_ssid().empty())
  this->add_sta(ap);
this->sta_.reserve(this->default_sta_.size()+1); // Reserve space first
  this->sta_.insert(this->sta_.end(),this->default_sta_.begin(),this->default_sta_.end());
}
...
void WiFiComponent::enableAp()
{
  ...
  this->ap_last_start_time_ = millis();
...
bool WiFiComponent::isApActive()
{
  return this->ap_active_;
}
